...
export default function handleFiles(files: Map<string, string>, onsC: string, dueDate: string) {
    ...
    for (let fileMap of files.entries()) {
        ...
        if (!fs.existsSync(`${downloadPath}/${fileOnsF}`)) {
            fs.mkdirSync(`${downloadPath}/${fileOnsF}`)
        }
    ...
    fs.readdirSync(downloadPath)
        .filter(file => {
            if (fs.existsSync(`${downloadPath}/${file}`)) return fs.statSync(`${downloadPath}/${file}`).isDirectory();
        })
        .forEach(dir => {
